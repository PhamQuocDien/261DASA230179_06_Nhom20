...
public:
    LoanSlipService(LoanRepository& loanRepository, MemberRepository& memberRepository, BookRepository& bookRepository);
    std::vector<LoanSlip> getLoanSlipsByMember(const std::string& memberId) const;
    bool getLoanSlipByLoanId(const std::string& loanId, LoanSlip& result) const;
};
#endif